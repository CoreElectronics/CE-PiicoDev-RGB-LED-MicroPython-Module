...
class PiicoDev_RGB:
	...
	def clear(self):d=self.i2c.readfrom_mem(self.addr,_regCtrl,1);r=int.from_bytes(d,_A);sleep_ms(1);r|=1<<0;self.i2c.writeto_mem(self.addr,_regCtrl,bytes([r]));sleep_ms(1)
	...
	def pwrLED(self,bit):
		d=self.i2c.readfrom_mem(self.addr,_regCtrl,1);d=int.from_bytes(d,_A);sleep_ms(1)
		if bit==False:d&=~(1<<1)
		else:d|=1<<1
